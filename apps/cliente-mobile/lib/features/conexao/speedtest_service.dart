...
class SpeedtestService {
  ...
  Future<double> download(void Function(double mbps) onProgress) async {
    ...
    try {
      ...
      throw SpeedtestFalhou('O teste de download demorou demais.', e);
    } catch (e) {
  ...
  /// POST de um corpo de 10MB pseudo-aleatório à Cloudflare; reporta janelas
  /// de progresso via [onProgress] usando o onSendProgress do dio; retorna
  /// a média geral em Mbps.
  Future<double> upload(void Function(double mbps) onProgress) async {
    final token = _newToken();
    final body = _randomBytes(_uploadBytes);
    final overallStart = DateTime.now();
    var lastSent = 0;
    var windowStart = DateTime.now();
    try {
    ...
    final overallElapsed = DateTime.now().difference(overallStart);
    return mbpsFrom(body.length, overallElapsed);
  }
